...
$parameters = array('clientversion', 'attemptid', 'testclassname', 'studentscode',
		'junitcode', 'memory_xmx', 'memory_limit_output', 'timeoutreal');

foreach ($parameters as $val) {
	if (!isset($_POST[$val])) {
		header("HTTP/1.1 400 Bad Request");
		write_log('[error 400] bad request from ' . $_SERVER ["REMOTE_ADDR"]);
		die("Bad Request: not all needed post parameters given");
	}
}
...
write_log('[questionattemptid ' . intval($_POST['attemptid']) .'] request took ' .
 ...
 * @param string $cmd
 *          command on shell
 * @param int $timeout_real
 *          timeout in secs (real time)
 * @param int $output_limit
 *          stops the process if the output on stdout/stderr reaches a limit (in Bytes)
 * @param string &$output
 *          stdout/stderr of process
 * @param float &$time
 *          time needed for execution (in s)
 * @return int
 *          OPEN_PROCESS_SUCCESS, OPEN_PROCESS_TIMEOUT, OPEN_PROCESS_OUTPUT_LIMIT
 *          or OPEN_PROCESS_OTHER_ERROR
*/
function open_process($cmd, $timeout_real, $output_limit, &$output, &$time) {
	...
	$orig_pipes = array($pipes[1], $pipes[2]);
...
function compile_execute() {
	...
	try {
		...
		if ($fd_testfile === false) throw new Exception('could not create testfile');
		...
		if ($fd_studentsfile === false) throw new Exception('could not create studentsfile');
		...
		if (!empty($compiler['compileroutput'])) {
			...
			return array('error' => true, 'errortype' => 'COMPILE_STUDENT_ERROR', 'compileroutput' => $compileroutput);
		}
		...
		if (!empty($compiler['compileroutput'])) {
			...
			return array('error' => true, 'errortype' => 'COMPILE_TESTFILE_ERROR');
		}

		// run test
		$command = PATH_TO_JAVA . ' -Xmx' . $memory_xmx .
			'm -Djava.security.manager=default -Djava.security.policy=' .
			PATH_TO_POLICY . ' -cp ' . PATH_TO_JUNIT . ':' . PATH_TO_HAMCREST .
			':' . $temp_folder . ' org.junit.runner.JUnitCore ' . $testclassname;
		...
		if ($ret_proc == OPEN_PROCESS_TIMEOUT || $ret_proc == OPEN_PROCESS_UNCAUGHT_SIGNAL) {
			...
			return array('error' => true, 'errortype' => 'TIMEOUT_RUNNING');
		}
 ...
 *
 * @param string $file
 *        	the .java file that should be compiled
 * @return string $compileroutput the output of the compiler
 */
function compile($file) {
	global $memory_limit_output, $timeoutreal;
	
	$command = PATH_TO_JAVAC . ' -nowarn -cp ' . PATH_TO_JUNIT . ' -sourcepath ' .
			dirname($file) . ' ' . $file;

	// execute the command
	$compileroutput = '';
	$time = 0;
	$ret = open_process(PRECOMMAND . ';' . escapeshellcmd($command), $timeoutreal,
			$memory_limit_output * 1024, $compileroutput, $time);
